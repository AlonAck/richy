...
function Goals(props) {
  var _f = useState({ name: "", target: "", saved: "", deadline: "", linkKey: "" });
  var form = _f[0]; var setForm = _f[1];
  var _a = useState(null);
  var addSheet = _a[0]; var setAddSheet = _a[1];
  var _am = useState("");
  var addAmt = _am[0]; var setAddAmt = _am[1];
  var _amo = useState("add");
  var addMode = _amo[0]; var setAddMode = _amo[1];
  var _ca = useState(null);
  var confirmAmt = _ca[0]; var setConfirmAmt = _ca[1];
  var _eg = useState(null);
  var editGoal = _eg[0]; var setEditGoal = _eg[1];
  var _ef = useState({ name: "", target: "", deadline: "", linkKey: "" });
  var editForm = _ef[0]; var setEditForm = _ef[1];
  ...
  return (
    <div>
      <Overlay open={props.sheetOpen} onClose={function() { props.setSheetOpen(false); }} title={tr("newBudgetBook")}>
        ...
        <FormRow label={tr("alreadySaved")} value={form.saved} onChange={function(e) { setField("saved", e.target.value); }} type="number" />
        <FormRow label={tr("deadline")} value={form.deadline} onChange={function(e) { setField("deadline", e.target.value); }} type="date" last={true} />
        {syncSelectRow(form.linkKey, function(v) { setField("linkKey", v); })}
        <BigBtn label={tr("createBudgetBook")} disabled={!form.name || !form.target} onPress={function() {
          var lk = parseLinkKey(form.linkKey);
          props.onSaveGoals(props.goals.concat([{ id: Date.now(), name: form.name, target: parseFloat(form.target), saved: parseFloat(form.saved) || 0, deadline: form.deadline || "", linkType: lk.linkType, linkId: lk.linkId }]));
          setForm({ name: "", target: "", saved: "", deadline: "", linkKey: "" });
          props.setSheetOpen(false);
        }} />
      </Overlay>
      ...
      <Overlay open={!!editGoal} onClose={function() { setEditGoal(null); }} title={tr("editBudgetBook")}>
        ...
        <FormRow label={tr("target")} value={editForm.target} onChange={function(e) { setEditField("target", e.target.value); }} type="number" />
        <FormRow label={tr("deadline")} value={editForm.deadline} onChange={function(e) { setEditField("deadline", e.target.value); }} type="date" last={true} />
        {syncSelectRow(editForm.linkKey, function(v) { setEditField("linkKey", v); })}
        <BigBtn label={tr("saveChanges")} disabled={!editForm.name || !editForm.target} onPress={function() {
          ...
          if (editGoal && editForm.name && n > 0) {
            var lk = parseLinkKey(editForm.linkKey);
            props.onSaveGoals(props.goals.map(function(g) {
              return g.id === editGoal.id ? Object.assign({}, g, { name: editForm.name, target: n, deadline: editForm.deadline || "", linkType: lk.linkType, linkId: lk.linkId }) : g;
            }));
          }
          setEditGoal(null);
        }} />
      ...
      <Overlay open={!!addSheet} onClose={function() { setAddSheet(null); setAddAmt(""); setAddMode("add"); setConfirmAmt(null); }} title={addSheet ? addSheet.name : ""}>
        {confirmAmt != null ? (
          <div>
            <div style={{ fontSize: 15, color: T.ink, lineHeight: 1.5, marginBottom: 18 }}>
              {tr("removeMoneyConfirm").replace("{amt}", dollars(confirmAmt)).replace("{name}", addSheet ? addSheet.name : "")}
            </div>
            <BigBtn label={tr("yesDo")} color={T.red} onPress={function() {
              props.onSaveGoals(props.goals.map(function(g) {
                return g.id === addSheet.id ? Object.assign({}, g, { saved: Math.max(0, g.saved - confirmAmt) }) : g;
              }));
              setAddSheet(null); setAddAmt(""); setAddMode("add"); setConfirmAmt(null);
            }} />
            <button onClick={function() { setConfirmAmt(null); }}
              style={{ width: "100%", background: "none", border: "none", color: T.ink2, fontSize: 14, fontWeight: 600, fontFamily: UI, cursor: "pointer", marginTop: 12, padding: "6px 0" }}>
              {tr("notNow")}
            </button>
          </div>
        ) : (
          <div>
            ...
            <BigBtn label={addMode === "remove" ? tr("removeFromBudgetBook") : tr("addToBudgetBook")} color={addMode === "remove" ? T.red : undefined} disabled={!addAmt} onPress={function() {
              var n = parseFloat(addAmt);
              if (!n || !addSheet) return;
              if (addMode === "remove") {
                setConfirmAmt(n);
              } else {
                props.onSaveGoals(props.goals.map(function(g) {
      ...
      {props.goals.map(function(g) {
        ...
        if (g.deadline) {
          ...
          var overdueGoal = !done && g.deadline < todayISOg;
          var dDate = new Date(g.deadline + "T00:00:00");
          var dText = isNaN(dDate.getTime()) ? "" : dDate.toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" });
          ...
          <Card key={g.id} style={{ marginBottom: 14, overflow: "hidden" }}>
            <div style={{ padding: "18px 18px 14px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                <div>
                  ...
                  {deadlineBadge && !done && (
                    <div style={{ marginTop: 6 }}>
                      <span style={{ fontSize: 10, fontWeight: 700, borderRadius: 5, padding: "1px 6px", letterSpacing: "0.02em", color: deadlineBadge.overdue ? T.red : T.ink3, background: deadlineBadge.overdue ? T.redDim : T.inputBg }}>
                        {(deadlineBadge.overdue ? tr("overdue") + " - " : tr("due") + " ") + deadlineBadge.text}
                      </span>
                    </div>
                  )}
                </div>
              ...
              <ProgressBar value={saved} max={g.target} color={done ? T.green : T.orange} h={6} />
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6 }}>
                <span style={{ fontSize: 11, color: T.ink3 }}>{dollars(saved)}</span>
            ...
            <div style={{ display: "flex", borderTop: "0.5px solid " + T.sep }}>
              {(function() {
                var btns = [];
                if (!done && !isLinked) {
                  btns.push({ key: "add", label: "+ " + tr("add"), color: T.orange, bold: true, onClick: function() { setAddSheet(g); setAddAmt(""); setAddMode("add"); setConfirmAmt(null); } });
                }
                if (saved > 0 && !isLinked) {
                  btns.push({ key: "remove", label: tr("removeMoney"), color: T.red, bold: true, onClick: function() { setAddSheet(g); setAddAmt(""); setAddMode("remove"); setConfirmAmt(null); } });
                }
...
function Trips(props) {
  ...
  // Rename a budget category or recolor it - doesn't touch planned/spent, so no
  // syncTripTx needed (the linked balance tx only cares about total spend).
  function updateTripAllocMeta(tripId, key, label, color) {
    var lbl = (label || "").trim();
    if (!lbl) return;
    var nextTrips = props.trips.map(function(t) {
      if (t.id !== tripId) return t;
      var allocs = t.allocations.map(function(a) { return a.key === key ? Object.assign({}, a, { label: lbl, color: color || a.color }) : a; });
      return Object.assign({}, t, { allocations: allocs });
    });
    props.onSaveTrips(nextTrips);
    setEditAllocFor(null);
  }
  ...
  function detailView(trip) {
    ...
    return (
      <div>
            ...
            <Card key={a.key} style={{ marginBottom: 12, overflow: "hidden" }}>
              <div style={{ padding: "15px 16px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 11, marginBottom: 10 }}>
                  <button onClick={function() { setEditAllocFor({ tripId: trip.id, key: a.key, icon: a.icon }); setEditAllocForm({ label: a.label, color: a.color }); }}
                    style={{ flex: 1, minWidth: 0, display: "flex", alignItems: "center", gap: 11, background: "none", border: "none", padding: 0, cursor: "pointer", textAlign: "left", fontFamily: UI }}>
                    <div style={{ width: 34, height: 34, borderRadius: 10, background: a.color + "1F", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      <SVGIcon id={a.icon} size={18} color={a.color} />
                    </div>
                    <span style={{ flex: 1, minWidth: 0, fontSize: 15, fontWeight: 600, color: T.ink, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{a.label}</span>
                  </button>
        ...
        <Overlay open={!!editAllocFor} onClose={function() { setEditAllocFor(null); }} title={tr("editCategory")}>
          ...
          <BigBtn label={tr("saveChanges")} disabled={!editAllocForm.label.trim()} onPress={function() {
            if (editAllocFor) updateTripAllocMeta(editAllocFor.tripId, editAllocFor.key, editAllocForm.label, editAllocForm.color);
          }} />
        </Overlay>
...
function Advisor(props) {
  ...
  function monthsUntilDate(dateStr) {
    ...
    var today = new Date();
    var months = (d.getFullYear() - today.getFullYear()) * 12 + (d.getMonth() - today.getMonth());
    if (d.getDate() < today.getDate()) months -= 1;
    return months;
  }
  function fmtDeadlineShort(dateStr) {
    var d = new Date(dateStr + "T00:00:00");
  ...
  function renderAnalysisHero() {
    ...
    if (goals.length > 0) {
      ...
      var paceNeeded = (paceGoal && !paceOverdue) ? paceGap / Math.max(1, paceMonthsLeft) : 0;
      var paceShort = Math.max(0, Math.round(paceNeeded - surplus));
      var paceStatus = !paceGoal ? "none" : (paceOverdue ? "overdue" : (surplus >= paceNeeded ? "ontrack" : "behind"));
      var paceColor = paceStatus === "ontrack" ? HPOS : (paceStatus === "none" ? HFNT : HNEG);
      var paceBadge = paceStatus === "ontrack" ? "ON TRACK" : paceStatus === "behind" ? "BEHIND PACE" : paceStatus === "overdue" ? "PAST DUE" : "NO DEADLINE";

      panels.push(
        <div key="pace" style={panelStyle({ justifyContent: "space-between" })}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontSize: 11, letterSpacing: "0.1em", textTransform: "uppercase", fontWeight: 600, color: HMUT }}>Goal Pace</span>
            <span style={{ background: paceColor + "26", color: paceColor, fontSize: 11, fontWeight: 700, letterSpacing: "0.04em", textTransform: "uppercase", padding: "4px 10px", borderRadius: 8 }}>{paceBadge}</span>
          </div>
          {paceGoal ? (
            <div>
              <div style={{ display: "flex", alignItems: "baseline", gap: 9 }}>
                <span style={{ fontSize: 36, fontWeight: 700, letterSpacing: "-0.03em", color: HINK, lineHeight: 1 }}>{dollars(paceNeeded)}</span>
                <span style={{ fontSize: 13, color: HFNT }}>needed / mo</span>
              </div>
              <div style={{ fontSize: 13, color: HMUT, marginTop: 9, lineHeight: 1.5 }}>
                {paceStatus === "overdue"
                  ? (paceGoal.name + "'s deadline has passed with " + dollars(paceGap) + " still to go. Set a new deadline or step up your pace.")
                  : paceStatus === "ontrack"
                  ? ("You're keeping " + dollars(surplus) + "/mo - enough to fund " + paceGoal.name + " on time.")
                  : ("You're keeping " + dollars(surplus) + "/mo, " + dollars(paceShort) + " short of what " + paceGoal.name + " needs to land on time.")}
              </div>
            </div>
          ) : (
            <div style={{ fontSize: 15.5, fontWeight: 600, color: HINK, lineHeight: 1.4 }}>
              Give a goal a deadline and I'll tell you if your pace will get you there.
            </div>
          )}
          <div style={{ borderTop: "0.5px solid " + HSEP, paddingTop: 14 }}>
            {paceGoal ? (
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                <span style={{ fontSize: 12.5, fontWeight: 600, color: HINK, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: "58%" }}>{paceGoal.name}</span>
                <span style={{ fontSize: 12, color: HMUT }}>{fmtDeadlineShort(paceGoal.deadline) + (paceOverdue ? "" : (paceMonthsLeft > 0 ? " - " + paceMonthsLeft + " mo left" : " - due this month"))}</span>
              </div>
            ) : (
              <div style={{ fontSize: 12.5, color: HMUT }}>{goals.length + " active goal" + (goals.length === 1 ? "" : "s") + ", none with a deadline yet."}</div>
            )}
          </div>
        </div>
      );
