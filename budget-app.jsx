...
function Goals(props) {
  var _f = useState({ name: "", target: "", saved: "", deadline: "" });
  var form = _f[0]; var setForm = _f[1];
  ...
  var _ef = useState({ name: "", target: "", deadline: "" });
  var editForm = _ef[0]; var setEditForm = _ef[1];
    ...
    <div>
      <Overlay open={props.sheetOpen} onClose={function() { props.setSheetOpen(false); }} title={tr("newBudgetBook")}>
        ...
        <BigBtn label={tr("createBudgetBook")} disabled={!form.name || !form.target} onPress={function() {
          props.onSaveGoals(props.goals.concat([{ id: Date.now(), name: form.name, target: parseFloat(form.target), saved: parseFloat(form.saved) || 0, deadline: form.deadline || "" }]));
        ...
        <BigBtn label={tr("saveChanges")} disabled={!editForm.name || !editForm.target} onPress={function() {
          ...
          if (editGoal && editForm.name && n > 0) {
            props.onSaveGoals(props.goals.map(function(g) {
              return g.id === editGoal.id ? { id: g.id, name: editForm.name, target: n, saved: g.saved, deadline: editForm.deadline || "" } : g;
            }));
          }
          setEditGoal(null);
        }} />
        ...
        {confirmAmt != null ? (
          <div>
            ...
            <BigBtn label={tr("yesDo")} color={T.red} onPress={function() {
              props.onSaveGoals(props.goals.map(function(g) {
                return g.id === addSheet.id ? { id: g.id, name: g.name, target: g.target, saved: Math.max(0, g.saved - confirmAmt), deadline: g.deadline || "" } : g;
              }));
          ...
          <div>
            ...
            <BigBtn label={addMode === "remove" ? tr("removeFromBudgetBook") : tr("addToBudgetBook")} color={addMode === "remove" ? T.red : undefined} disabled={!addAmt} onPress={function() {
              ...
              if (addMode === "remove") {
                ...
                props.onSaveGoals(props.goals.map(function(g) {
                  return g.id === addSheet.id ? { id: g.id, name: g.name, target: g.target, saved: Math.min(g.target, g.saved + n), deadline: g.deadline || "" } : g;
                }));
                setAddSheet(null);
                setAddAmt("");
              }
          ...
          <Card key={g.id} style={{ marginBottom: 14, overflow: "hidden" }}>
            ...
            <div style={{ display: "flex", borderTop: "0.5px solid " + T.sep }}>
              {(function() {
                ...
                btns.push({ key: "edit", label: tr("edit"), color: T.ink2, bold: false, onClick: function() { setEditGoal(g); setEditForm({ name: g.name, target: String(g.target), deadline: g.deadline || "" }); } });
