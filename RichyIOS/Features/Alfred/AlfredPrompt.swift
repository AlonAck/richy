...
/// The system prompt for Alfred's chat on the phone, built from the live
/// ledger at send time. Same shape as the web app's chat prompts: the
/// person's own background notes first (facts, never rules), Alfred's
...
enum AlfredPrompt {
    ...
    static func system(store: LedgerStore, user: AuthUser) -> String {
        var parts: [String] = []
        if let notes = store.account?.alfredInstructions?.trimmingCharacters(in: .whitespacesAndNewlines), !notes.isEmpty {
    ...
    /// The web app's `alfredUserCtx`: authoritative for facts about the
    /// person's life, never for Alfred's rules.
    ...
    static let persona = """
    You are Alfred, the warm, sharp money guide inside the Richy app, talking to the user in the iPhone app. \
