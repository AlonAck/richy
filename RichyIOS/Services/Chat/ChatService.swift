...
/// One round-trip to Alfred through `POST /api/chat`. The server owns the
...
protocol ChatService: Sendable {
    func reply(to messages: [ChatMessage],
               system: String,
               maxTokens: Int,
               model: AlfredModel) async throws -> String
}
...
final class RichyChatService: ChatService {
    ...
    func reply(to messages: [ChatMessage],
               system: String,
               maxTokens: Int = 800,
               model: AlfredModel = .core) async throws -> String {
...
final class MockChatService: ChatService {
    func reply(to messages: [ChatMessage],
        ...
        return "This is a demo reply. Connect Firebase to talk to the real Alfred."
    }
}
