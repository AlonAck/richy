...
- ~~**P0 — four net-worth formulas, three different answers.** `motivSnapshot`
  ...
  from all four sites.**~~ **CLOSED 11 Sep.** `netWorthOf(state)` now lives at
  `:2884` and is the only net-worth formula in the app. Six call sites, not four —
  the audit missed the dashboard `netWorth` widget tile and the `linkType ===
  "networth"` branch of `goalSavedAmount`, both of which carried their own copy.
  Verified end-to-end through the compiled production bundle: on a test account
  worth $10,200, moving $1,000 into an own savings pot used to push Profile to
  $11,200 while every other screen held at $10,200, and 60 shuffles of the user's
  own money read $610,200 and awarded all six `h-0*` badges. After the fix all six
  screens read $10,200 flat through every shuffle and only `h-01`/`h-02` — the two
  the real $10,200 against a $5,000 opening genuinely earns — are awarded.
  Internal transfers net out because the main-ledger side of the transfer is
  counted, not dropped; external pot deposits, real spend, trip lump-sums and
  held shares still move the number, and pending, future and `catchUp` rows still
  do not (18/18 regression checks).
...
were never wired to a button (`RW_ACTIONS` — partially wired 7 Sep, `3af9b1c`,
budget-pace dismiss only, see Closed table above; and `deleteTripConfirm`, which was
...
| ~~161 physical directional CSS properties (marginLeft/Right, paddingLeft/Right, borderLeft/Right, textAlign:"left"/"right") plus 67 absolute left:/right: offsets don't flip under dir="rtl", so translated Hebrew/Arabic screens get correct text direction but broken spacing/alignment~~ (§7, partial — the five highest-traffic screens only, not all ~161 sites across the app) | `61e4a79` — converted 26 sites across Overview/Activity/Budgets/Goals/Advisor to logical properties (marginInlineStart/End, paddingInlineStart/End, borderInlineStart/End, textAlign: start/end, insetInlineStart/End). Verified by re-reading all five screens' full source for zero remaining physical-property matches. The other ~119 screens are unchanged - this was scoped to "highest-traffic first," not a full sweep. |
| ~~`IntroCarousel` hardcodes `$2,840 / $412 / $95 / $203` next to translated Hebrew category labels, before the user has picked a currency~~ (§7) | `6f59521` — dropped currency symbols from all three mock slides in all four languages, not just the balance card the report named - the goal/trip chips on slides 2-3 had the identical issue. Verified by re-reading the whole component and its `ic*` translation keys: zero `$` remain. |
| ~~`theme-color` had three different values: `index.html` (`#8970C6` purple), `manifest.webmanifest` (`#141210` near-black), and the app's real light background (`#F7F3EE` cream) - light-mode Android showed a purple status bar over a cream app~~ (§7) | `a289fac` — set `index.html`'s default meta and both `manifest.webmanifest` fields to `#F7F3EE`. Traced `#141210` to the dead literal in the `T` object's own declaration rather than `DARK_BG` (`#131110`), the constant `applyDarkMode()` actually uses - confirmed the boot script's existing dark-mode override already matches `DARK_BG` exactly, so dark mode needed no change. |
| ~~`ErrorBoundary`'s only action was `location.reload()`, which re-triggers the same crash when it's caused by persisted local state, with no "reset local data" escape hatch~~ (§7, escape-hatch half only — telemetry still goes to `console.error` only, and the copy is still hardcoded English) | `9f9a848` — added `resetLocalDataAndReload()` as a secondary "Still stuck?" button: clears `localStorage` and every IndexedDB database under the origin, then reloads. Verified with an isolated Node test running the real function against mocked browser APIs. |
| ~~`BadgeTile`'s locked state rendered a lock icon and no name, and `BadgeGlyph` used `alt=""` - 157 completely unlabelled buttons on the badge wall~~ (§7, badge wall only - the other ~267 unlabelled buttons app-wide and the 0-`htmlFor`/0-input-`id` gap are still open) | `d7a74e7` — `BadgeTile` now has an `aria-label` ("{name}, {rarity} badge, unlocked" / "Locked badge, not yet earned", never naming a locked badge) and `BadgeGlyph` a real `alt`. Verified by calling the real function for both an unlocked and a locked badge and inspecting the built props. |
